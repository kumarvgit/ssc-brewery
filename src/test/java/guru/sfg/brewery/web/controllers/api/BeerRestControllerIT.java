...
class BeerRestControllerIT extends BaseIT {
    ...
    public class BeerDeleteTests{
        ...
        @MethodSource("guru.sfg.brewery.web.controllers.BaseIT#getStreamOfSuperAdmin")
        void deleteBeerNoAuthParameterizedTest(String user, String pwd) throws Exception {
            mockMvc.perform(delete("/api/v1/beer/" + beerToDelete().getId())
    ...
    void findBeers() throws Exception {
        mockMvc.perform(get("/api/v1/beer/").with(anonymous())).andExpect(status().isUnauthorized());
    }
    ...
    void findBeerById() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/beer/" + beer.getId()).with(anonymous())).andExpect(status().isUnauthorized());
    }
    ...
    // Find beer by UPC
    @Test
    void findBeerUpcByUpcNoAuth() throws Exception {
        mockMvc.perform(get("/api/v1/beerUpc/0631234200036").with(anonymous())).andExpect(status().isUnauthorized());
    }
    @Test
    void findBeerUpcByUpcAdmin() throws Exception {
        mockMvc.perform(get("/api/v1/beerUpc/0631234200036").with(httpBasic("spring", "guru")))
                .andExpect(status().isOk());
    }

    @Test
    void findBeerUpcByUpcCustomer() throws Exception {
        mockMvc.perform(get("/api/v1/beerUpc/0631234200036").with(httpBasic("spring", "guru")))
                .andExpect(status().isOk());
    }

    @Test
    void findBeerUpcByUpcUser() throws Exception {
        mockMvc.perform(get("/api/v1/beerUpc/0631234200036").with(httpBasic("spring", "guru")))
                .andExpect(status().isOk());
    }
    ...
    @ParameterizedTest(name = "#{index} with [{arguments}]")
    @MethodSource("guru.sfg.brewery.web.controllers.BaseIT#getStreamOfAdmin")
    void findBeerUpcByUpc(String user, String pwd) throws Exception{

        mockMvc.perform(get("/api/v1/beerUpc/0631234200036").with(httpBasic(user, pwd)))
                .andExpect(status().isOk());
    }
}