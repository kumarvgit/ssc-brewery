...
public class UserController {
    ...
    public String register2fa(Model model){
        ...
        User user = getUser();
        // this url is going to retrieve back a QR code
        String url = GoogleAuthenticatorQRGenerator.getOtpAuthURL("SFG", getUser().getUsername(),
    ...
    public String verifyPostOf2Fa(@RequestParam Integer verifyCode){
        ...
        if (googleAuthenticator.authorizeUser(user.getUsername(), verifyCode)) {
            // setting 2fa to false sine it is already verified in this method
    ...
    public String confirm2Fa(@RequestParam Integer verifyCode){
        ...
        if (googleAuthenticator.authorizeUser(user.getUsername(), verifyCode)) {
            ...
            // being safe if we get stale object
            User savedUser = userRepository.findById(user.getId()).orElseThrow();
            savedUser.setUseGoogle2fa(true);
            userRepository.save(savedUser);
            return "index";
        } else {
            // registration failed and return back to registration page
            return "user/register2fa";
        }
    }
